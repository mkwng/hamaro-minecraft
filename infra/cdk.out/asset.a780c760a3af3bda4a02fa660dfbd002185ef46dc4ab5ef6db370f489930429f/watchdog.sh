#!/usr/bin/env bash
# Runs every 20s (hamaro-watchdog.timer). Two speeds in one script:
#   - FAST, every tick: player-count check (a lightweight Server List Ping,
#     not RCON) + idle tracking + heartbeat publish. This is what the website
#     reads, so keeping it fast keeps the status page feeling live.
#   - SLOW, ~once a minute (self-timed, not tick-counted): the RCON-heavy
#     stuff — map position sync, death pins, whitelist-knock detection. These
#     open real RCON connections logged in the server console, so they stay
#     on the same cadence as before; only the cheap heartbeat got faster.
# Idle time is tracked in real elapsed SECONDS (not "ticks"), so the shutdown
# threshold (IDLE_MINUTES) means the same thing regardless of tick frequency.
set -uo pipefail
source /etc/hamaro/env

NOW=$(date +%s)
LAST_TICK=$(cat /run/hamaro-last-tick 2>/dev/null || echo "$NOW")
DELTA=$((NOW - LAST_TICK)); [ "$DELTA" -lt 0 ] && DELTA=0
echo "$NOW" > /run/hamaro-last-tick

UPTIME_MIN=$(awk '{print int($1/60)}' /proc/uptime)
IDLE_SEC_FILE=/run/hamaro-idle-sec
IDLE_SEC=$(cat "$IDLE_SEC_FILE" 2>/dev/null || echo 0)
...
echo "$IDLE_SEC" > "$IDLE_SEC_FILE"
IDLE_MIN_DISPLAY=$((IDLE_SEC / 60))

PROFILE=$(grep '^PROFILE=' /srv/minecraft/runtime.env 2>/dev/null | cut -d= -f2 || echo "")

# ---- slow path: only once every ~60s, however often we're ticked ----
LAST_ENRICH=$(cat /run/hamaro-last-enrich 2>/dev/null || echo 0)
RUN_ENRICH=false
if [ $((NOW - LAST_ENRICH)) -ge 55 ]; then
  RUN_ENRICH=true
  echo "$NOW" > /run/hamaro-last-enrich
fi
...
DEATHS
  ...
  # across the 15s overlap between the 75s log window and the ~60s enrich cadence.
  if [ "$STATE" = "running" ]; then
...
HEARTBEAT_JSON=$(PROFILE="$PROFILE" STATE="$STATE" PLAYERS="${PLAYERS:-}" IDLE="$IDLE_MIN_DISPLAY" \
...
if [ "$IDLE_SEC" -ge "$((IDLE_MINUTES * 60))" ]; then
  echo "[watchdog] ${IDLE_MIN_DISPLAY} idle minutes — backing up and shutting down"
