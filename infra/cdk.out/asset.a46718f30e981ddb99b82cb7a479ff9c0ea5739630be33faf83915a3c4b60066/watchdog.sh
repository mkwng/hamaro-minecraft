...
# Publish live player markers for the public terrain map (best effort).
if [ -n "$PLAYERS" ] && [ "$PLAYERS" -gt 0 ] 2>/dev/null; then
  MARKERS="UnminedCustomMarkers = { isEnabled: true, markers: ["
  ...
  for P in $NAMES; do
    ...
    X=$(echo $POS | cut -d' ' -f1 | cut -d. -f1); Z=$(echo $POS | cut -d' ' -f3 | cut -d. -f1)
    [ -n "$X" ] && MARKERS="$MARKERS{x: $X, z: $Z, text: \"$P\", font: \"bold 16px sans-serif\", color: \"gold\"},"
  done
  echo "${MARKERS}]}" > /tmp/markers.js
else
  echo "UnminedCustomMarkers = { isEnabled: true, markers: [] }" > /tmp/markers.js
fi
aws s3 cp /tmp/markers.js "s3://${SITE_BUCKET}/map/custom.markers.js" \
